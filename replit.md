...
- **Lead Intelligence System V2**: Multi-feature lead generation platform with 6 sub-tabs (Dashboard, Search, Sources, Campaigns, Templates, Webhooks). **Search Pipeline** (10 steps): (1) AI Query Builder, (2) Discovery (DDG+Bing), (3) Fetch/Parse, (4) Deep Crawl directories, (5) Contact Page Follow, (6) AI Extraction (GPT-4o-mini with 2nd pass), (6b) AI Enrichment (website scraping + finstat.sk/orsr.sk/ARES/WKO.at, SSRF-protected), (7) Deduplication, (8) Scoring, (9) Save, (10) Auto-track source domains. **Source Management**: CRUD with quality scoring (0-100), whitelist/blacklist dropdown, JS-heavy site detection, recalculate-all scoring, XLSX export. Tables: `lead_sources`. **Automated Campaigns**: Scheduled recurring searches (daily/weekly/monthly), direct function call execution, mutex-protected scheduler. Tables: `lead_campaigns`. **Lead Dashboard**: Analytics with stats, breakdowns, timeline. **V2 Features**: (1) Multilingual query templates (SK/CZ/DE/AT/HU/RO/IT) with built-in seed data, usage tracking, one-click apply. Tables: `query_templates`. (2) Advanced source scoring algorithm (success rate, avg results, response time, JS-heavy penalty, whitelist/blacklist boost). (3) Manual merge/reject/approve workflow for search results with bulk actions, status filter, CRM merge button. (4) XLSX export for results and sources. (5) Blacklist/whitelist management per source. **V3 Features**: (1) Webhooks (CRUD + test + HMAC-SHA256 signing) for result.reviewed, result.merged, campaign.completed events. Tables: `webhook_configs`. (2) Multi-source enrichment (finstat.sk, orsr.sk, ARES, WKO.at, firmenbuch.at, DNS validation). (3) Intelligent source sampling (weighted random by quality score). Countries: SK, CZ, AT, HU, RO, IT, DE, US, CH. Endpoints: `/api/query-templates`, `/api/webhooks`, `/api/search-results/:id/review`, `/api/search-results/:id/merge`, `/api/search-results/bulk-review`, `/api/search-results/export/:jobId`, `/api/lead-sources/export`, `/api/lead-sources/:id/recalculate-score`, `/api/lead-sources/recalculate-all`, `/api/lead-sources/:id/list-type`, `/api/lead-sources/smart-sample`, `/api/search-results/:id/enrich`.
