...
- **Status Management Engine**: Comprehensive, configurable status/disposition system with 9 categories (Nedovolané, Callback, Záujem, Zmluva, Email/SMS, Materiály, Odmietnutie, Dokončené, Chybné kontakty), 104 pre-seeded statuses, 14 disposition action types, and per-status meta rules (isFinal, isConversion, requiresNote, requiresCallback, allowRecontact, allowPhone/Email/SMS, isSystemStatus, visibleInCampaigns). Admin UI at `/status-management` with category management, status editing, search/filter, and seed/reset functionality. DB tables: `status_categories`, `status_definitions`. API routes: `/api/status-categories/*`, `/api/status-definitions/*`. Seed data: `server/seed-statuses.ts`.
