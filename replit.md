...
- **SOP Management**: Admin page (`/sop`) for creating and managing Standard Operating Procedures. Features: category management with icons and sort order, rich text HTML articles with priority levels (normal/high/critical), pinning, country filtering, read tracking per user, campaign linking (articles can be associated with specific campaigns for contextual display in agent workspace), and publish/draft status. Tables: `sop_categories`, `sop_articles`, `sop_article_reads`, `sop_campaign_articles`.
