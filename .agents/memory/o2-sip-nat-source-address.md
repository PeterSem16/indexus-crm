...
For this O2 trunk, the provider required the explicit auth realm `o2bssk` and a registrar URI without the `:5060` suffix. Registration succeeded when the password was placed directly in the active trunk configuration without delimiter quotes; the exact contribution of moving it versus removing quotes was not isolated.