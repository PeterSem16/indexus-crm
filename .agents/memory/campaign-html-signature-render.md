...
## Correction: for EMAILS `entry.campaignId` is ALWAYS undefined — only `selectedCampaignId` matters
**Why:** the `communication_messages` table (source of email/SMS history rows) has NO `campaignId` column, and the server contact-history handler only sets `campaignId` on CALL rows, never email/SMS rows. So no client mapping change can populate `entry.campaignId` for emails — the reply resolution effectively always falls back to `selectedCampaignId`.
**How to apply:** the per-campaign reply signature therefore ONLY appears when the agent is actively working THAT campaign (selectedCampaignId is set at task-open). If the signature was configured on campaign X but the agent replies while working campaign Y, it correctly won't show — this is expected, not a bug. Don't chase `entry.campaignId` for emails; it can't be made to work without adding a schema column (avoided on this prod).
