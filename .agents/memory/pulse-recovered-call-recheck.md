...
Treat recovery as provisional for a short stability window. If SIP terminates without an explicit agent hangup or server-confirmed customer hangup while an interruption is unresolved—or shortly after apparent recovery—reassert the deferred readiness reason so Oops appears after wrap-up.