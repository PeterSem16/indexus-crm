...
- **Where to add it:** the final INVITE to SLOVANET is generated by the OLD asterisk (SLOVANET-trunk box, chan_sip), context `from-mediagtw` → `Dial(SIP/SLOVANET-VGW/…)`. Add `SIPAddHeader(Diversion: …)` there BEFORE the Dial (escape `;` as `\;` in extensions.conf), guarded with `ExecIf($["${CALLERID(num):0:7}" != "0232399"]?…)` so normal outbound presenting our own DDI is not marked as diverted. Reload: `asterisk -rx "dialplan reload"`. This is a server-side dialplan change, NOT in the repo.
- **Fallback = option 2:** present own DID `0232399030` as CLI (loses original caller identity; SLOVANET confirmed fully authorized).
