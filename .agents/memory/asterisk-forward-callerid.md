...
**Next experiment:** add a second header in the guarded block: `SIPAddHeader(P-Asserted-Identity: <sip:0232399030@212.55.232.229>)` (PAI = our authorized number, From = original caller; telco-standard combo with Diversion). Outcomes: partner validates PAI → may display From (ideal) or display PAI (shows our DID — still better than anonymous). Risk: if peer config has sendrpid=pai there'd be two PAI headers — check with `sip set debug peer SLOVANET-VGW`. If PAI reintroduces 486, remove the line. Otherwise → report to SLOVANET (connects but anonymous; what else do they need to display original CLI).
