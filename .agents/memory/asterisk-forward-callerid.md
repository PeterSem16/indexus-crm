...
### IMPLEMENTED (current decision): DID-substitution ONLY on the standing path — NO notifications
The chosen tradeoff lives **only in `connectCallToStandingAgent`** (the sole live queue→mobile forward
path). A central `resolveForwardCli(realCaller, did)` presents the company queue DID as the CLI **only for
SK (+421) callers** (`isSlovakInternationalCaller` + `toNationalSkDid` → national `0…` form); foreign
callers keep their real number verbatim. This is what makes SK forwards **connect reliably** ("vždy sa
presmeruje"). Trade-off the user accepted as the fallback: the SK caller's mobile shows the company DID,
not the real caller.

**⚠️ REMOVED — do NOT re-introduce without an explicit request:** the earlier out-of-band caller delivery
(in-app + Expo push + SMS, `server/lib/agent-call-alert.ts`, `alertAgentIncomingCall`, the
`recentAgentCallAlerts` dedup map, `cliWasSubstituted` gate) was **deleted at the user's explicit demand**
("tie notifikácie nechcem, je to blbosť"). The whole module + all references are gone.

**Deliberately NOT applied** to `forwardToExternalNumber` (collaborator / queue forward-number): keeps the
real number there (SK calls on that path stay as-is — pre-existing, acceptable).

### Refined root cause (architect-validated) — say this to the user, not "wholesale carrier"
- The `10.9.33.2` "DialLog.Dialer" (SDP: `Asterisk PBX 13.21.0-vici`) peer is **the company's OWN legacy
  SK/ViciDial server**, not necessarily the wholesale carrier. The 180/183 early media are generated by
  THAT box; the 486 is relayed from behind it — could be the real SK operator OR rules on that DialLog box
  they control. Worth one check of that box's own config/logs.
- "Nothing changed but it broke" is best explained by **Slovak anti-spoofing regulation (enforcement phases
  through 2025–2026)** requiring operators to block calls presenting Slovak CLIs over unauthorized/
  interconnect routes — zero config change needed on their side. Do NOT assert the pre-existing forward path
  "used an owned CLI" — no evidence; standing-forward is new so there may be no comparable "before".
- Real SK-caller display + connect is a genuine conflict, fixable only carrier-side: **CLIP no-screening**
  authorization, or route SK→SK forwards via a legit **Slovak operator trunk**. PAI/RPID do NOT bypass
  screening (network-asserted identity is exactly what gets screened). One cheap untested lever worth a
  single live test: **Diversion header** (RFC 5806) — From=real caller + `Diversion: <owned DID>;reason=
  unconditional`; some operators whitelist SK CLIs with a valid owned-number Diversion, but the legacy
  Asterisk-13/chan_sip next hop likely strips it, so low probability — not worth blocking on.
