...
**Interpretation:** we've done format + Diversion exactly per SLOVANET's spec; the 486 still comes from the interconnect partner. Genuinely partner-side now — nothing more to change on our dialplan without SLOVANET's input.

### ✅ Jul-8 21:54 UPDATE: the 486 is TRANSIENT — foreign CONNECTS on auto-retry (looks like anti-spam greylisting)
Fresh foreign test (`00491723627488` + Diversion, UNIQUEID `1783540474.1297`, 21:54:34): **1st outbound attempt → 486**, then INDEXUS/queue-engine auto-requeued and **2nd attempt ANSWERED + bridged** — caller ID preserved. So option 3 EFFECTIVELY WORKS for foreign; the partner rejects the first INVITE and accepts the retry (classic reject-once-accept-retry anti-spam / greylisting). **The retry is done by the NEW asterisk (our requeue loop), not the old-box dialplan** — a fresh `from-mediagtw` call arrives after each 486, so our existing retry logic is what saves it (it already treats 486/busy as retryable).
**Still to confirm:** SK CLI `0911163316` — in the 21:40 log it got 486 on several retries and the log was truncated before any connect, so SK anti-spam may be STRICTER (on-net-looking SK number from another SK network) than foreign. Re-test SK and watch whether a later retry connects.
**⚠️ Testing gotcha (Jul-8 22:33):** a "SK still 486 forever" log turned out to be a SELF-CALL — inbound `FROM=0421948519438` (=`0948519438`) forwarded OUT to target `0948519438` (same phone). A phone calling itself is genuinely busy → permanent legitimate 486, NOT the antispoof 486. ALWAYS check caller (from-sk-inbound `FROM=`) ≠ forward target (from-mediagtw `EXTEN=`) before interpreting a 486. Valid SK test needs a DIFFERENT SK source number → a DIFFERENT agent mobile.
**Decision tree:** if SK also connects on retry → keep option 3 for everything (caller ID preserved, retry absorbs the greylisting). If SK never connects → either ask SLOVANET/partner to whitelist our Diversion/CLI, OR use option 2 (present own DID `0232399030`) for SK numbers only while keeping option 3 for foreign. To reduce the first-attempt delay long-term, the clean fix is partner-side whitelisting; the retry is the working stopgap.

### Jul-9 08:33 SK TEST: connects (no 486 at all, 1st attempt) but B-party displays "ANONYMOUS"
Caller `0903282745` (Orange) → DID `0232399030` → forward `0948519438` (O2). UNIQUEID old-box `1783578812.12`, new-box `1783578812.605`. INVITE had From=0903282745 + Diversion(privacy=off) → call CONNECTED immediately, but the mobile showed "anonymous call" instead of the CLI. So the partner's antispoof has (at least) 3 observed modes: 486-reject (T-Mobile CLI 0911… Jul-8 evening), greylist-then-accept (foreign 0049… — connected 2nd attempt), and deliver-but-ANONYMIZE (Orange CLI 0903… Jul-9 morning). Behavior varies by CLI network and/or time — partner-side logic, we send correctly.
**PAI experiment (Jul-9) → ⚠️ SUPERSEDED by Slovanet's Jul-9 reply below:** with Diversion+PAI both sent, the first-attempt 486/greylist DISAPPEARED — foreign `00491723627488` (UNIQUEID `1783579895.100`, 08:51) and SK both connect on the 1st attempt now. We concluded PAI fixed the 486/greylisting — WRONG (confounded variable, see below): the SK test number changed between days. What PAI did NOT fix (display): added `SIPAddHeader(P-Asserted-Identity: <sip:0232399030@212.55.232.229>)` after Diversion; header confirmed sent; call still connects but B-party STILL shows "anonymous" (neither original CLI nor our DID displayed). Conclusion: we now send From(original) + Diversion + PAI(own DID) textbook-style and the anonymization happens downstream (SLOVANET gateway or partner strips/marks it) — **nothing more fixable via our SIP headers**. Remaining paths: (a) SLOVANET/partner must pass the CLI (report + their O2-SIM test), or (b) option 2 for forwards: in the guarded block replace both SIPAddHeader lines with `Set(CALLERID(num)=0232399030)` — own number displays reliably (it's ours), kills antispoof/greylist 486 too; agent sees company number, real caller shown by INDEXUS app push. Reversible one-liner.

### ✅ Jul-9 SLOVANET AUTHORITATIVE REPLY (ball is in SLOVANET's court — no changes needed on our side)
Slovanet analyzed the Jul-9 test calls and answered in writing:
- **Our Diversion header IS received correctly** in the INVITE at their gateway — but their OWN network strips it before it reaches the target operator (their gateway doesn't map it onward). This happens inside Slovanet's equipment and is THEIR bug to analyze/fix. Net effect: the target operator still never sees "this call is diverted" — same as before.
- **No CLIR is requested by us or added by Slovanet** — the anonymization happens downstream at the receiving/transit operator.
- **The Jul-8 vs Jul-9 difference was the TEST NUMBER's actual network, NOT PAI:** Jul-8 tests used `0911163316` (active in T-Mobile) → 486; Jul-9 used `0903282745` (0903 = T-Mobile prefix but PORTED to Orange) → passes with CLIR. Partner antispoof behaves differently per the caller number's CURRENT network (portability matters, not the prefix): T-Mobile-active CLI → 486 reject; Orange-active CLI → connect but CLIR'd. Slovanet is near-certain a T-Mobile CLI retest would still 486 even with Diversion.
- **PAI is DISCARDED by Slovanet — zero effect.** Optional to remove; harmless to keep. (Our "PAI fixed the greylisting" conclusion is dead — do not act on it.)
- **Foreign (+49) anonymization theory:** most likely the receiving/transit operator's antispoof sees a foreign CLI from an SK operator WITHOUT diversion info → treats it as possible spoofing → CLIRs it. If diversion info propagates (once Slovanet fixes their gateway mapping), both SK and foreign display should be solved together.
- **Next steps (all Slovanet's):** (1) they test with their own O2 SIM to confirm the diversion-info hypothesis; (2) they fix why Diversion doesn't propagate from the voice gateway through their network. **Nothing to change on our side** — keep sending From(original)+Diversion(+optional PAI); wait for their update.
**Rule: do NOT experiment with more SIP header changes while Slovanet's analysis is pending** — the blocker is confirmed inside Slovanet's network, and antispoof results are confounded by which network the test CLI actually lives in (always record the caller number's REAL network, not its prefix, in future tests).
