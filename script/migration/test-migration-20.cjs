  ...
  try {
    ...
    try {
      const tables = [
        ...
        { table: 'customer_potential_cases', where: "data_source = 'iscbc'" },
        { table: 'collection_results', where: "collection_id IN (SELECT id FROM collections WHERE data_source = 'iscbc')" },
        { table: 'collections', where: "data_source = 'iscbc'" },
        { table: 'collaborator_activities', where: "legacy_id IS NOT NULL AND legacy_id LIKE 'cbc_cc_%'" },
