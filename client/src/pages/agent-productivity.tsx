...
import { Gauge, HelpCircle, Users } from "lucide-react";
...
export default function AgentProductivityPage() {
  ...
  const { data: rows = [], isLoading } = useQuery<AgentProductivityRow[]>({
    queryKey: ["/api/reports/agent-productivity", fromISO, toISO],
    enabled: validRange,
    queryFn: async () => {
      const res = await fetch(
        `/api/reports/agent-productivity?from=${encodeURIComponent(fromISO)}&to=${encodeURIComponent(toISO)}`,
        { credentials: "include" },
      );
  ...
  return (
    <div className="space-y-4" data-testid="page-agent-productivity">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold" data-testid="text-page-title">{ap.title}</h1>
