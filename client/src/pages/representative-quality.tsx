...
import {
  ...
  Zap, AlertCircle, AlertTriangle, CheckCircle, ListChecks, Loader2,
  UserCircle2, Stethoscope
} from "lucide-react";
...
// ── MetricCard ─────────────────────────────────────────────────────────────────
function MetricCard({
  icon: Icon, title, value, unit, light, sub, loading
}: {
  icon: React.ElementType; title: string; value: string;
  unit?: string; light: Light; sub?: string; loading?: boolean;
}) {
  return (
    <Card className={`overflow-hidden transition-all duration-300 ${RING[light]}`}>
...
// ── Main page ──────────────────────────────────────────────────────────────────
export default function RepresentativeQualityPage() {
  const { t } = useI18n();
  ...
  const selectedRep = useMemo(() => representatives.find((u: any) => u.id === repId), [representatives, repId]);
  ...
  const p90Light  = trafficLight(dq?.latency?.p90Hours,    24, 72, false);
  ...
  return (
    <div className="min-h-screen bg-background">
      {/* ── Hero header ───────────────────────────────────────────────────── */}
      <div className="relative overflow-hidden bg-gradient-to-br from-violet-600 via-purple-600 to-indigo-700 text-white px-6 py-8">
        ...
        <div className="relative max-w-5xl mx-auto">
          <div className="flex items-center gap-3 mb-2">
            ...
            <div>
              <h1 className="text-2xl font-black tracking-tight">{rq.pageTitle}</h1>
              <p className="text-violet-200 text-sm mt-0.5">{rq.pageDesc}</p>
            </div>
          </div>
          {selectedRep && (
            <div className="mt-4 inline-flex items-center gap-2 bg-white/15 backdrop-blur-sm rounded-full px-4 py-1.5 text-sm font-semibold">
              <UserCircle2 className="h-4 w-4" />
              {selectedRep.firstName} {selectedRep.lastName}
              <span className="w-px h-3 bg-white/40 mx-1" />
              <CalendarDays className="h-3.5 w-3.5 text-violet-200" />
              <span className="text-violet-200">{formatRange()}</span>
            </div>
          )}
        </div>
      </div>
          ...
          <Card className="overflow-hidden">
            <CardContent className="pt-4 pb-4">
              <label className="text-xs font-bold uppercase tracking-widest text-muted-foreground mb-2 flex items-center gap-1.5">
                <Users className="h-3.5 w-3.5" /> {rq.selectRep}
              </label>
            ...
            <CardContent className="pt-4 pb-4">
              ...
              <Popover open={calOpen} onOpenChange={setCalOpen}>
                <PopoverTrigger asChild>
                  <Button variant="outline" className="h-11 w-full justify-between text-sm font-medium">
        ...
        {!repId && (
          <Card className="border-dashed border-2">
            <CardContent className="py-16 text-center">
              ...
              <p className="text-lg font-semibold text-muted-foreground">{rq.pickRepFirst}</p>
            </CardContent>
          </Card>
        )}
                    ...
                    <table className="w-full text-xs">
                      <thead className="sticky top-0 bg-muted/80 backdrop-blur-sm">
                        <tr>
                          ...
                          <th className="text-left px-4 py-2 font-semibold">Chýba / Missing</th>
                        </tr>
                      </thead>
                      <tbody>
                              ...
                              {dq.consistency.noCallStatuses.map((r: any, i: number) => (
                                <tr key={r.clinicId + r.confirmedAt} className={i % 2 === 0 ? "" : "bg-muted/20"}>
                                  ...
                                  <td className="px-3 py-1.5 text-muted-foreground">
                                    {new Date(r.confirmedAt).toLocaleDateString("sk-SK", { day: "numeric", month: "numeric", hour: "2-digit", minute: "2-digit" })}
                                  </td>
                                </tr>
                              ))}
                            </tbody>
