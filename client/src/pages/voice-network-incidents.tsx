...
type Incident = { id: string; callLogId?: string | null; customerName?: string | null; kind: string; severity: string; connectionState?: string | null; iceState?: string | null; rttMs?: number | null; jitterMs?: number | null; packetLossPermille?: number | null; createdAt: string; userName: string };
type Result = { items: Incident[]; total: number; page: number; pageSize: number };
const kinds = ["browser_offline", "sip_transport_disconnected", "sip_registration_disconnected", "ice_failed", "ice_disconnected_sustained", "audio_no_flow", "audio_one_way", "audio_device_change_failed", "network_quality_degraded"];
const incidentLabels: Record<string, string[]> = {
  en: ["Browser offline", "SIP transport disconnected", "SIP registration disconnected", "ICE failed", "ICE disconnected (sustained)", "No audio flow", "One-way audio", "Audio device switch failed", "Network quality degraded", "Warning", "Error"],
  sk: ["Prehliadač je offline", "SIP transport odpojený", "SIP registrácia odpojená", "ICE zlyhalo", "ICE odpojené (trvalo)", "Žiadny tok zvuku", "Jednosmerný zvuk", "Prepnutie audio zariadenia zlyhalo", "Zhoršená kvalita siete", "Upozornenie", "Chyba"],
  cs: ["Prohlížeč je offline", "SIP transport odpojen", "SIP registrace odpojena", "ICE selhalo", "ICE odpojeno (trvale)", "Žádný tok zvuku", "Jednosměrný zvuk", "Přepnutí audio zařízení selhalo", "Zhoršená kvalita sítě", "Upozornění", "Chyba"],
  hu: ["A böngésző offline", "SIP szállítás megszakadt", "SIP-regisztráció megszakadt", "ICE hiba", "ICE megszakadt (tartósan)", "Nincs hangforgalom", "Egyirányú hang", "Az audioeszköz-váltás sikertelen", "Romló hálózati minőség", "Figyelmeztetés", "Hiba"],
  ro: ["Browser offline", "Transport SIP deconectat", "Înregistrare SIP deconectată", "ICE eșuat", "ICE deconectat (persistent)", "Fără flux audio", "Audio unidirecțional", "Schimbarea dispozitivului audio a eșuat", "Calitate degradată a rețelei", "Avertisment", "Eroare"],
  it: ["Browser non in linea", "Trasporto SIP disconnesso", "Registrazione SIP disconnessa", "ICE non riuscito", "ICE disconnesso (prolungato)", "Nessun flusso audio", "Audio unidirezionale", "Cambio dispositivo audio non riuscito", "Qualità rete degradata", "Avviso", "Errore"],
  de: ["Browser offline", "SIP-Transport getrennt", "SIP-Registrierung getrennt", "ICE fehlgeschlagen", "ICE getrennt (anhaltend)", "Kein Audiofluss", "Einseitiges Audio", "Audiogerätewechsel fehlgeschlagen", "Netzwerkqualität beeinträchtigt", "Warnung", "Fehler"],
};
...
export default function VoiceNetworkIncidentsPage({ embedded = false }: { embedded?: boolean }) {
  const { user } = useAuth();
  ...
  const quickFilters = {
    en: ["Today", "This week", "This month", "Custom range", "Customer"],
    sk: ["Dnes", "Tento týždeň", "Tento mesiac", "Vlastný rozsah", "Zákazník"],
    cs: ["Dnes", "Tento týden", "Tento měsíc", "Vlastní rozsah", "Zákazník"],
    hu: ["Ma", "Ez a hét", "Ez a hónap", "Egyéni időtartomány", "Ügyfél"],
    ro: ["Astăzi", "Săptămâna aceasta", "Luna aceasta", "Interval personalizat", "Client"],
    it: ["Oggi", "Questa settimana", "Questo mese", "Intervallo personalizzato", "Cliente"],
    de: ["Heute", "Diese Woche", "Dieser Monat", "Benutzerdefinierter Zeitraum", "Kunde"],
  }[locale] || ["Today", "This week", "This month", "Custom range", "Customer"];
  const applyPeriod = (next: "today" | "week" | "month" | "range") => {
    setPeriod(next);
    setPage(1);
  ...
  const url = useMemo(() => {
    ...
    if (to) {
      ...
      p.set("to", exclusiveEnd.toISOString());
    }
  ...
  return <div className="space-y-4">
    {!embedded && <PageHeader title={copy[0]} description={copy[1]} />}
    <Card><CardContent className="pt-6 flex flex-wrap gap-3 items-end">
      <div className="flex flex-wrap gap-2">{(["today", "week", "month", "range"] as const).map((value, index) => <Button key={value} type="button" variant={period === value ? "default" : "outline"} onClick={() => applyPeriod(value)}>{quickFilters[index]}</Button>)}</div>
       {period === "range" && <><div><Label>{copy[2]}</Label><Input type="date" value={from} onChange={e => { setFrom(e.target.value); setPage(1); }} /></div>
    ...
    <Card><CardContent className="pt-6 overflow-x-auto"><Table><TableHeader><TableRow><TableHead>{copy[9]}</TableHead><TableHead>{copy[7]}</TableHead><TableHead>{copy[4]}</TableHead><TableHead>{copy[5]}</TableHead><TableHead>{quickFilters[4]}</TableHead><TableHead>{metricHeaders[1]}</TableHead><TableHead>{metricHeaders[2]}</TableHead><TableHead>{metricHeaders[3]}</TableHead><TableHead>{metricHeaders[4]}</TableHead></TableRow></TableHeader><TableBody>
