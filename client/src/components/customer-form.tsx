...
import { CalendarIcon, Copy, PhoneCall, User, MapPin, Briefcase, Building2, FileText } from "lucide-react";
...
function CardLayoutForm({ form, initialData, isHidden, isReadonly, complaintTypes, cooperationTypes, vipStatuses, filteredHealthInsurance, selectedCountry, useCorrespondenceAddress, copyToClipboard, t }: any) {
  const getDobFields = () => {
    const currentDate = form.watch("dateOfBirth") || null;
    ...
    return { currentYear, currentMonth, currentDay, years, months, days, getDaysInMonth };
  };
                ...
                <FormField control={form.control} name="healthInsuranceId" render={({ field }) => (
                  <FormItem><FormLabel>{t.customers.fields.healthInsurance}</FormLabel>
                    <Select onValueChange={(val) => field.onChange(val === "__none__" ? null : val)} defaultValue={field.value || "__none__"} disabled={isReadonly("health_insurance")}>
                      ...
                      <SelectContent>
                        <SelectItem value="__none__">{t.customers.none}</SelectItem>
                        {filteredHealthInsurance.filter((hi: any) => hi.isActive).map((ins: any) => (<SelectItem key={ins.id} value={ins.id}>{ins.name} ({ins.code})</SelectItem>))}
                      </SelectContent>
                    </Select>
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
        {useCardLayout ? (
          <CardLayoutForm
            ...
            filteredHealthInsurance={filteredHealthInsurance}
            selectedCountry={selectedCountry}
            useCorrespondenceAddress={useCorrespondenceAddress}
