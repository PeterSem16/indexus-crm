...
export function PulseDiagnostics({ open, required = false, keepWakeLock = false, hasValidReadiness = false, autoStartRequest = 0, userId, onClose, onReady, onExit }: Props) {
  ...
  const presentation = getPulsePresentationState(state, running, runCompleted, canContinue, hasValidReadiness);
  const { isBeginning, showDetails: showDiagnosticDetails } = presentation;
  const acknowledge = () => {
            ...
            {presentation.showHeader && <DialogHeader><DialogTitle className="flex items-center gap-3 text-xl tracking-tight"><span className="flex h-10 w-10 items-center justify-center rounded-2xl bg-primary/10 text-primary ring-1 ring-primary/15"><ShieldCheck className="h-5 w-5" /></span><span>{t.title}</span></DialogTitle><DialogDescription className="pl-13">{t.subtitle}</DialogDescription></DialogHeader>}
             ...
             {isBeginning && <section className="relative overflow-hidden rounded-2xl border border-primary/20 bg-gradient-to-br from-primary/[0.10] via-primary/[0.035] to-background px-5 py-8 text-center sm:px-10 sm:py-11" aria-labelledby="pulse-beginning-title">
               ...
               <div className="relative">
                 <div className="flex flex-col items-center gap-2"><span className="flex h-14 w-14 items-center justify-center rounded-[1.25rem] bg-primary text-primary-foreground shadow-lg shadow-primary/20"><ShieldCheck className="h-6 w-6" /></span><div className="text-[10px] font-bold uppercase tracking-[0.16em] text-primary/70">{t.readinessIntroEyebrow}</div><div className="text-xs text-muted-foreground">{t.readinessIntroDuration}</div></div>
                 <h2 id="pulse-beginning-title" className="mx-auto mt-5 max-w-lg text-2xl font-bold tracking-tight sm:text-3xl">{t.readinessIntroTitle}</h2>
                 <p className="mx-auto mt-3 max-w-xl text-sm leading-relaxed text-muted-foreground">{t.readinessIntroDetail}</p>
                 <div className="mx-auto mt-6 flex max-w-md items-center justify-center gap-2 rounded-xl border border-primary/15 bg-background/60 px-3 py-2.5 text-xs font-medium text-muted-foreground"><Mic className="h-4 w-4 shrink-0 text-primary" />{t.readinessIntroPrivacy}</div>
                 <Button size="lg" className="mt-6 h-12 w-full rounded-xl font-bold shadow-lg shadow-primary/20 sm:w-auto sm:px-7" onClick={() => void run()} data-testid="button-pulse-start"><Play className="h-4 w-4 fill-current" />{t.start}</Button>
               </div>
             </section>}
