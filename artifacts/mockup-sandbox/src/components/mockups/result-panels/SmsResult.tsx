...
import {
  MessageSquare, ChevronDown, ChevronUp, ChevronRight, CircleDot,
  Send, Clock, CheckCheck, MessageSquareOff, Star, User, Target,
  CalendarPlus, RefreshCw, Phone
} from "lucide-react";
...
const categories = [
  {
    ...
    color: "#2E75B6",
    statuses: [
      { id: "s1", name: "SMS odoslaná — čakáme na odp.", icon: Send, action: "send_sms", days: null },
      { id: "s2", name: "SMS prečítaná — follow-up 2d", icon: CheckCheck, action: "schedule_sms", days: "2d" },
      { id: "s3", name: "Opakovaná SMS — týždeň", icon: RefreshCw, action: "schedule_sms", days: "1t" },
    ],
  },
  {
    id: "cat2",
    ...
    color: "#5B7A4E",
    statuses: [
      { id: "s4", name: "Záujem — zákazník zavolá späť", icon: Phone, action: "callback", days: null },
      { id: "s5", name: "Termín dohodnutý cez SMS", icon: CalendarPlus, action: "complete", days: null },
    ],
  },
  {
    id: "cat3",
    ...
    color: "#7A6858",
    statuses: [
      { id: "s6", name: "Číslo neexistuje / chyba", icon: MessageSquareOff, action: "dnd", days: null },
      { id: "s7", name: "Žiadna odpoveď — 3 dni", icon: Clock, action: "schedule_sms", days: "3d" },
      { id: "s8", name: "Žiadna odpoveď — 2 týždne", icon: Clock, action: "schedule_sms", days: "2t" },
    ],
  },
  {
    id: "cat4",
    ...
    color: "#A0493A",
    statuses: [
      { id: "s9", name: "Odhlásiť z SMS (STOP)", icon: MessageSquareOff, action: "dnd", sub: 2 },
    ],
  },
];
...
export function SmsResult() {
  ...
  return (
    <div className="min-h-screen flex flex-col" style={{ background: STONE_BG, fontFamily: "system-ui, sans-serif" }}>
      {/* Header */}
      <div style={{ background: WHITE, borderBottom: `1px solid ${SMS_AC}30`, padding: "16px 20px" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 6 }}>
          <div style={{
            width: 34, height: 34, borderRadius: 10,
            background: SMS_AC, display: "flex", alignItems: "center", justifyContent: "center",
            boxShadow: `0 2px 8px ${SMS_AC}40`
          }}>
            <Target style={{ width: 16, height: 16, color: WHITE }} />
          </div>
          <div>
            <div style={{ fontSize: 15, fontWeight: 700, color: TEXT_PRIMARY }}>Výsledok SMS</div>
            <div style={{ fontSize: 11, color: TEXT_MUTED }}>Vyberte výsledok pred pokračovaním</div>
          </div>
        </div>
        <div style={{
          display: "flex", alignItems: "center", gap: 6,
          background: `${SMS_AC}10`, borderRadius: 8, padding: "6px 10px",
          border: `1px solid ${SMS_AC}25`
        }}>
          <User style={{ width: 12, height: 12, color: SMS_AC }} />
      ...
      <div style={{ padding: "10px 16px 4px", display: "flex", alignItems: "center", gap: 6 }}>
        <MessageSquare style={{ width: 12, height: 12, color: SMS_AC }} />
      ...
      {/* Notes area */}
      <div style={{ padding: "8px 12px 16px", borderTop: `1px solid ${SMS_AC}20`, background: WHITE, marginTop: 4 }}>
        <div style={{ fontSize: 11, fontWeight: 600, color: TEXT_MUTED, marginBottom: 6, textTransform: "uppercase", letterSpacing: "0.06em" }}>
          Poznámka k SMS (voliteľné)
        </div>
        <div style={{
          borderRadius: 10, border: `1px solid ${SMS_AC}25`, background: CARD_BG,
