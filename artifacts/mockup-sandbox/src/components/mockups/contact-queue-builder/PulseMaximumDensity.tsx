      ...
      <section style={{ maxWidth: 1440, margin: "0 auto", background: "#fbfdff", border: `1px solid #caddeb`, borderRadius: 14, boxShadow: "0 20px 52px rgba(28,67,103,.14)", overflow: "hidden" }}>
        <header style={{ minHeight: 58, display: "flex", alignItems: "center", gap: 12, padding: "0 18px", borderBottom: `1px solid ${colors.line}`, background: "#f8fbfe" }}>
         ...
         <div style={{ minHeight: 440 }}>
           <div style={{ padding: "12px 16px 10px", borderBottom: `1px solid ${colors.line}`, background: "#ffffff" }}>
             <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
               <label style={{ display: "flex", alignItems: "center", gap: 7, flex: 1, minWidth: 180, height: 31, padding: "0 10px", border: `1px solid ${colors.line}`, borderRadius: 6, background: colors.pale, color: colors.muted }}><Search size={13} /><input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search name, phone, email..." aria-label="Search scheduled queue" style={{ border: 0, outline: 0, width: "100%", fontSize: 11, background: "transparent", color: colors.ink }} />{query && <button type="button" onClick={() => setQuery("")} style={{ border: 0, background: "transparent", color: colors.muted, cursor: "pointer" }}><X size={12} /></button>}</label>
               <span style={{ fontSize: 9, color: colors.muted, letterSpacing: ".08em", textTransform: "uppercase", fontWeight: 800 }}>Sort by</span>
               <div style={{ display: "flex", alignItems: "center", gap: 3 }}>{(["Date", "Name", "Campaign"] as const).map((item) => <button key={item} type="button" onClick={() => activateSort(item)} style={{ display: "flex", alignItems: "center", gap: 3, height: 29, border: `1px solid ${sort === item ? "#c4dced" : colors.line}`, borderRadius: 5, background: sort === item ? colors.mint : "#fff", color: sort === item ? colors.tealDark : colors.muted, padding: "0 7px", fontSize: 10, cursor: "pointer", fontWeight: sort === item ? 700 : 500 }}>{item}{sort === item ? direction === "up" ? <ArrowUp size={10} /> : <ArrowDown size={10} /> : <ArrowUpDown size={10} />}</button>)}</div>
             </div>
             <div style={{ display: "flex", alignItems: "center", flexWrap: "wrap", gap: 6, marginTop: 10 }}>
               <span style={{ fontSize: 9, color: colors.muted, letterSpacing: ".08em", textTransform: "uppercase", fontWeight: 800, marginRight: 2 }}>Time</span>
               {timeFilters.map((filter) => <button key={filter} type="button" onClick={() => setTimeFilter(filter)} style={{ display: "inline-flex", alignItems: "center", gap: 5, height: 25, padding: "0 8px", border: `1px solid ${timeFilter === filter ? "#b9d8ed" : colors.line}`, borderRadius: 5, background: timeFilter === filter ? colors.mint : "#fff", color: timeFilter === filter ? colors.tealDark : colors.muted, fontSize: 9, fontWeight: timeFilter === filter ? 750 : 550, cursor: "pointer" }}><span style={{ color: filter === "overdue" ? colors.coral : colors.muted }}>{filter === "All" ? "▦" : filter === "overdue" ? "!" : filter === "today" ? "◷" : "□"}</span>{filter}<em style={{ fontStyle: "normal", color: timeFilter === filter ? colors.teal : colors.muted }}>{countForTime(filter)}</em></button>)}
               <span style={{ width: 1, height: 17, background: colors.line, margin: "0 3px" }} />
               <span style={{ fontSize: 9, color: colors.muted, letterSpacing: ".08em", textTransform: "uppercase", fontWeight: 800, marginRight: 2 }}>Type</span>
               {typeFilters.map((filter) => <button key={filter} type="button" onClick={() => setTypeFilter(filter)} style={{ display: "inline-flex", alignItems: "center", gap: 5, height: 25, padding: "0 8px", border: `1px solid ${typeFilter === filter ? "#b9d8ed" : colors.line}`, borderRadius: 5, background: typeFilter === filter ? colors.mint : "#fff", color: typeFilter === filter ? colors.tealDark : colors.muted, fontSize: 9, fontWeight: typeFilter === filter ? 750 : 550, cursor: "pointer" }}>{filter === "Calls" ? <Phone size={11} /> : filter === "Emails" ? <Mail size={11} /> : filter === "SMS" ? <MessageSquare size={11} /> : "◉"}{filter}<em style={{ fontStyle: "normal", color: typeFilter === filter ? colors.teal : colors.muted }}>{counts[filter]}</em></button>)}
           </div>
           <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", minHeight: 32, padding: "0 16px", borderBottom: `1px solid ${colors.line}`, background: "#f7fbfe", color: colors.muted, fontSize: 10 }}>
             <span>Showing <strong style={{ color: colors.ink }}>{visible.length}</strong> of <strong style={{ color: colors.ink }}>{contacts.length}</strong> scheduled contacts</span>
             <span style={{ display: "inline-flex", alignItems: "center", gap: 6 }}><span style={{ width: 6, height: 6, borderRadius: "50%", background: colors.green }} />Ready for shift</span>
           </div>
