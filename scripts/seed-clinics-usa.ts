...
async function seedUSAClinics() {
  console.log('Reading USA clinics CSV file...');
  
  const csvPath = path.join(process.cwd(), 'attached_assets', 'gyneko_all_US_1768833772048.csv');
  ...
  console.log('Importing USA clinics...');
  let successCount = 0;
  let errorCount = 0;
  let skippedCount = 0;
...
function parseAddress(address: string): { street: string | null, city: string | null, postalCode: string | null } {
  ...
  // US address format: "Street Number, City, PostalCode" or "Street Number, PostalCode (unit X)"
  let cleanAddress = address.replace(/\s*\(unit[^)]*\)/gi, '').trim();
...
seedUSAClinics()
  .then(() => {
    console.log('USA clinics seed script completed');
    process.exit(0);
  })
  .catch((error) => {
