...
async function seedRomaniaClinics() {
  console.log('Reading Romania clinics CSV file...');
  
  const csvPath = path.join(process.cwd(), 'attached_assets', 'gyneko_all_RO_1768834046663.csv');
...
seedRomaniaClinics().then(() => { console.log('Romania clinics seed completed'); process.exit(0); }).catch((e) => { console.error('Failed:', e); process.exit(1); });
