...
async function seedItalyClinics() {
  console.log('Reading Italy clinics CSV file...');
  
  const csvPath = path.join(process.cwd(), 'attached_assets', 'gyneko_all_IT_1768833469506.csv');
  ...
  console.log('Importing Italy clinics...');
  let successCount = 0;
  let errorCount = 0;
  let skippedCount = 0;
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    try {
      ...
      // Build notes from speciality and opening hours
...
function parseAddress(address: string): { street: string | null, city: string | null, postalCode: string | null } {
  ...
  // Italian address format: "Street Number, City, PostalCode" or "Street Number, PostalCode"
  ...
  // Look for postal code (5 digits in Italy)
  ...
  for (let i = 1; i < parts.length; i++) {
    ...
    if (match) {
      postalCode = match[1];
      // City is usually before or after postal code in same part
...
seedItalyClinics()
  .then(() => {
    console.log('Italy clinics seed script completed');
    process.exit(0);
  })
  .catch((error) => {
