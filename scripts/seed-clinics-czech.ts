...
async function seedCzechClinics() {
  console.log('Reading Czech clinics CSV file...');
  
  const csvPath = path.join(process.cwd(), 'attached_assets', 'gyneko_all_CZ_1768834023649.csv');
...
function parseAddress(address: string): { street: string | null, city: string | null, postalCode: string | null } {
  ...
  const postalRegex = /\b(\d{3}\s?\d{2})\b/;
  for (let i = 1; i < parts.length; i++) {
    const match = parts[i].match(postalRegex);
    if (match) {
      postalCode = match[1].replace(/\s/g, '');
...
seedCzechClinics().then(() => { console.log('Czech clinics seed completed'); process.exit(0); }).catch((e) => { console.error('Failed:', e); process.exit(1); });
