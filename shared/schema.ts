...
export const clinics = pgTable("clinics", {
  ...
  interestCooperation: text("interest_cooperation"),
  interestContract: text("interest_contract"),
  contractStatus: text("contract_status"),
  lastCallResult: text("last_call_result"),
  lastCallNote: text("last_call_note"),
  nextContactDate: timestamp("next_contact_date"),
  contractSentDate: timestamp("contract_sent_date"),
  contractReturnedDate: timestamp("contract_returned_date"),
...
export const campaigns = pgTable("campaigns", {
  ...
  defaultActiveTab: text("default_active_tab").default("phone"), // phone, script, email, sms, checklist - which tab opens first when agent activates contact
...
export const insertCampaignSchema = createInsertSchema(campaigns).omit({
  ...
  defaultActiveTab: z.enum(["phone", "script", "email", "sms", "checklist"]).optional().default("phone"),
...
export const campaignOperatorSettings = pgTable("campaign_operator_settings", {
  ...
  userId: varchar("user_id").notNull(),
  isActive: boolean("is_active").notNull().default(true),
  workloadWeight: integer("workload_weight").notNull().default(100),
  maxContactsPerDay: integer("max_contacts_per_day").default(50),
  dailyCallQuota: integer("daily_call_quota"),
  dailyEmailQuota: integer("daily_email_quota"),
  dailySmsQuota: integer("daily_sms_quota"),
...
export const insertCampaignOperatorSettingSchema = createInsertSchema(campaignOperatorSettings).omit({
  ...
  dailyCallQuota: z.number().int().min(0).optional().nullable(),
  dailyEmailQuota: z.number().int().min(0).optional().nullable(),
  dailySmsQuota: z.number().int().min(0).optional().nullable(),
  assignedCountries: z.array(z.string()).optional().default([]),
});
...
export const insertCampaignDispositionSchema = createInsertSchema(campaignDispositions).omit({
  ...
  isConversion: z.boolean().optional().default(false),
  isDefault: z.boolean().optional().default(false),
  isActive: z.boolean().optional().default(true),
...
export const collectionSprievodnyList = pgTable("collection_sprievodny_list", {
  ...
  childFirstName: text("child_first_name"),
  childGender: text("child_gender"),
  childBirthNumber: text("child_birth_number"),
  ...
  deliveryComplications: text("delivery_complications"),
  cordBloodVolume: text("cord_blood_volume"),
  cordClampTime: text("cord_clamp_time"),
  placentaWeight: text("placenta_weight"),
  numberOfBags: text("number_of_bags"),
  bagId: text("bag_id"),
  ...
  hospitalName: text("hospital_name"),
  hospitalDepartment: text("hospital_department"),
  donorSelection: text("donor_selection"),
  maternalBloodSample: text("maternal_blood_sample"),
  antibiotics: text("antibiotics"),
  infections: text("infections"),
  previousPregnancies: text("previous_pregnancies"),
  labTests: text("lab_tests"),
  transportConditions: text("transport_conditions"),
  motherSignature: text("mother_signature"),
...
export const virtualAgentConfigs = pgTable("virtual_agent_configs", {
  ...
  ttsModel: text("tts_model").notNull().default("tts-1"),
  gptModel: text("gpt_model").notNull().default("gpt-4o-mini"),
  gptTemperature: numeric("gpt_temperature").notNull().default("0.5"),
  gptMaxTokens: integer("gpt_max_tokens").notNull().default(100),
  ...
  websiteLastFetched: timestamp("website_last_fetched"),
  isActive: boolean("is_active").notNull().default(true),
  createdAt: timestamp("created_at").notNull().default(sql`now()`),
  updatedAt: timestamp("updated_at").notNull().default(sql`now()`),
});
...
export const campaignPhases = pgTable("campaign_phases", {
  ...
  transitionMode: text("transition_mode").notNull().default("manual"),
  autoTransitionSchedule: text("auto_transition_schedule"),
  lastAutoTransitionAt: timestamp("last_auto_transition_at"),
...
export const insertCampaignPhaseSchema = createInsertSchema(campaignPhases).omit({
  ...
  transitionMode: z.enum(["manual", "automatic", "ai_assisted"]).optional().default("manual"),
  autoTransitionSchedule: z.enum(["hourly", "daily", "weekly"]).optional().nullable(),
...
export const webForms = pgTable("web_forms", {
  ...
  titleFontFamily: text("title_font_family").notNull().default("inherit"),
  subtitleFontSize: text("subtitle_font_size").notNull().default("sm"),
  subtitleFontWeight: text("subtitle_font_weight").notNull().default("normal"),
  subtitleFontStyle: text("subtitle_font_style").notNull().default("normal"),
  subtitleFontFamily: text("subtitle_font_family").notNull().default("inherit"),
  sectionFontSize: text("section_font_size").notNull().default("xs"),
  sectionFontWeight: text("section_font_weight").notNull().default("bold"),
  sectionFontStyle: text("section_font_style").notNull().default("normal"),
  labelFontSize: text("label_font_size").notNull().default("sm"),
  ...
  confirmEmailShowData: boolean("confirm_email_show_data").notNull().default(true),
  confirmEmailFooter: text("confirm_email_footer"),
  confirmEmailSignature: text("confirm_email_signature"),
  isActive: boolean("is_active").notNull().default(true),
  createdBy: varchar("created_by"),
  createdAt: timestamp("created_at").notNull().default(sql`now()`),
  updatedAt: timestamp("updated_at").notNull().default(sql`now()`),
...
export const customerDebtCollection = pgTable("customer_debt_collection", {
  ...
  contractNumber: text("contract_number"),
  amount: text("amount"),
  debtAmount: text("debt_amount"),
  currency: text("currency"),
  overdue_invoices_count: integer("overdue_invoices_count"),
  oldestDueDate: timestamp("oldest_due_date"),
  newestDueDate: timestamp("newest_due_date"),
  companyName: text("company_name"),
  status: text("status"),
  phase: text("phase"),
  startDate: timestamp("start_date"),
...
export const insertCampaignStatusListItemSchema = createInsertSchema(campaignStatusListItems).omit({
  ...
  description: z.string().optional().nullable(),
  parentId: z.string().optional().nullable(),
  confirmationType: z.enum(["checkbox", "radio", "info", "auto"]).optional().default("checkbox"),
